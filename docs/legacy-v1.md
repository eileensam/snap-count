...
**`js/pages/`:**
- `index.js`: fetches all weeks up to `currentWeek` on first load for the leaderboard (skips refetch if `state.totalGames` is already populated; data only gets fresher via a hard `localStorage` clear), then renders table/dropdowns/chart (Chart.js, loaded via CDN in `index.html`) and wires up player/week/group-toggle listeners.
