...
**A scratch mockup needs its own `<meta name="viewport" content="width=device-width, initial-scale=1">`.** Without it, Chrome's mobile emulation (`isMobile: true`) falls back to the classic 980px default layout viewport for non-mobile-optimized pages, so `window.innerWidth` silently reports 980 instead of the requested width, no error, just a wider layout than intended. `index.html` already has this tag so this doesn't come up there, but a new standalone file for design review needs it added explicitly.

**When hand-assembling HTML via string concatenation (as opposed to writing it directly), verify every tag actually closes.** A missing `</style>` once caused a page to render as completely empty with zero errors anywhere, no console output, no pageerror event, nothing, because the browser silently treated the entire rest of the document (body, every script tag) as unclosed `<style>` raw text. This is a uniquely bad failure mode: no signal at all, not even a broken layout to look at. Before debugging "why won't this run," `grep -c '<style\|</style\|<script\|</script'` the generated output and confirm open/close counts actually match.
